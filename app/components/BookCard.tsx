  ...
  if (variant === 'horizontal') {
    return (
      ...
      >
        ...
        <Text className="font-bold text-sm mt-2 text-red-squat" numberOfLines={1}>{book.title}</Text>
        <Text className="text-xs text-black">{book.author}</Text>
      </Pressable>
    );
  }
    ...
    >
      ...
      <View className="flex-1 p-3 justify-between">
        <View>
          ...
          <Text className="text-sm font-bold text-black mt-1">Por {book.author}</Text>
        </View>
        <Text className="font-bold text-lg text-red-squat">${book.price}</Text>
      </View>
    </Pressable>
  );
};
