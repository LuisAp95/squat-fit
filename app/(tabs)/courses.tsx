...
export default function CoursesScreen() {
  ...
  return (
    <ScrollView className="flex-1 bg-white pt-16">
      <View className="px-6">
        <Text className="text-3xl font-bold text-red-squat mb-6">Cursos</Text>

        {/* Sección Mis Cursos */}
        <View className="mb-8">
          <View className="flex-row justify-between items-center mb-3">
            <Text className="text-xl font-bold text-purple-squat-1">Mis cursos</Text>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false}>
             {myFirstCourses.map(myFirstCourse => (
             <Pressable className="mr-4" onPress={() => router.push(`/course/${myFirstCourse.id}`)}>
            <Image 
              source={{ uri: myFirstCourse.image }}
              style={{ width: "100%", height: 192, borderRadius: 12 }}
              className="w-full h-48 rounded-xl"
            />
            <Text className="text-lg text-purple-squat-1 font-bold mt-2">{myFirstCourse.title}</Text>
          </Pressable>
           ))}
          </ScrollView>
        ...
        <View>
          <Text className="text-xl font-bold text-red-squat mb-3">Recomendados para ti</Text>
